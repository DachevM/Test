...
const searchTransactions = (array, obj) => {
  ...
  array.forEach((elem) => {
    let filtr;
    filtr = elem.split.filter(
      (item) =>
        item.accountName.includes(obj.searchtext) ||
        item.year == obj.year ||
  ...
  if (obj.sort.charAt(0) === "-") {
    newArr.sort((a, b) => (a[obj.sort] < b[obj.sort] ? 1 : -1));
  }
  newArr.sort((a, b) => (a[obj.sort] > b[obj.sort] ? 1 : -1));

  return newArr;
};
console.log(searchTransactions(a, object));
...
const getTotal = (array, obj) => {
  ...
  let total = 0;
  newArr.forEach((el) => el.split.forEach((item) => (total += item.absAmount)));
  return total;
};
console.log(getTotal(a, object));
// Имеется массив
//
...
// Обьект вида
// {
//     sort -  название поля по которому будет происходить сортировка [ в качестве поля для сортировки могут использоваться поля из основного обекта без вложенности ] направление сортировки определяется по первому символу например "-name" это сортировка по имени по убыванию "name"  это сортировка по имени по возрастанию
//     searchText - текст по которому надо найти элементы в которых в массиве сплит содержатся элементы в accountName
...
// Условия для фитрации работают по принципу или (если задано несколько условий надо чтобы хотя бы одно из них выполнялось )
//
//
