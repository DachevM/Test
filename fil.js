...
const searchTransactions = (array, obj) => {
  let res;
  res = array.sort((a, b) => (a[obj.sortBy] > b[obj.sortBy] ? 1 : -1));

  if (obj.searchText || obj.month || obj.year) {
    res = res.filter(
      (item) =>
        item.split[0].category.includes(obj.searchText) ||
        item.split[0].year == obj.year ||
        item.split[0].month == obj.month
    );
  } else if (obj.searchText && obj.year) {
    res = res.filter(
      (item) =>
        item.split[0].category.includes(obj.searchText) &&
        item.split[0].year == obj.year
    );
  } else if (obj.searchText && obj.month) {
    res = res.filter(
      (item) =>
        item.split[0].category.includes(obj.searchText) &&
        item.split[0].month == obj.month
    );
  } else if (obj.year && obj.month) {
    res = res.filter(
      (item) =>
        item.split[0].year == obj.year && item.split[0].month == obj.month
    );
  } else if (obj.year) {
    res = res.filter((item) => item.split[0].year == obj.year);
  } else if (obj.month) {
    res = res.filter((item) => item.split[0].month == obj.month);
  } else if (obj.searchText) {
    res = res.filter((item) => item.split[0].category.includes(obj.searchText));
  }
  if (obj.sortDir == "desc") res.reverse();
  return res;
};
console.log(searchTransactions(a, object));

// let total = 0;
// res.map((el) => (total += el.split[0].absAmount));
...
// Условия для фитрации работают по принципу или (если задано несколько условий надо)
//
//
